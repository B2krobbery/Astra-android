-- 1. Add reset_user_discovery_feed RPC
CREATE OR REPLACE FUNCTION public.reset_user_discovery_feed()
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Delete all interactions where user is actor or target
  DELETE FROM public.interactions WHERE actor_id = v_uid OR target_id = v_uid;

  -- Delete matches involving this user
  DELETE FROM public.matches WHERE user1_id = v_uid OR user2_id = v_uid;

  RETURN true;
END;
$$;

GRANT EXECUTE ON FUNCTION public.reset_user_discovery_feed() TO authenticated;

-- 2. Ensure RLS policies for DELETE
DROP POLICY IF EXISTS "Users can delete own interactions" ON public.interactions;
CREATE POLICY "Users can delete own interactions" ON public.interactions FOR DELETE USING (auth.uid() = actor_id);

DROP POLICY IF EXISTS "Users can delete own matches" ON public.matches;
CREATE POLICY "Users can delete own matches" ON public.matches FOR DELETE USING (auth.uid() = user1_id OR auth.uid() = user2_id);

-- 3. Update get_discovery_candidates RPC to include family_income matching profiles column count
